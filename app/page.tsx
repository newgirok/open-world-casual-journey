  ...
  return (
    <main ref={rootRef} className="bg-white">
      ...
      <header
        ...
        // 화면 전체 폭(inset-x-0)에 고정 108px 패딩만 주면, 화면이 넓어질수록 로고↔nav 사이
        // 빈 공간만 계속 늘어나며 콘텐츠가 오른쪽으로 쏠려 보임(실측: 1920px에서 확인됨).
        // 토스도 화면 폭에 따라 패딩이 108→128→160px로 같이 늘어나는 유동 구조라, 우리는
        // 콘텐츠를 max-w 컨테이너로 감싸 가운데 정렬해 더 견고하게 동일 효과를 냄
      ...
      >
        <div className="max-w-[1440px] mx-auto flex items-center justify-between h-16 px-gutter md:px-[108px]">
          ...
          {/* 토스 실측 구조 그대로: 로고/버튼이 양끝에 따로 있는 게 아니라, [내비게이션 링크 5개
              ...
              gap 255px는 1440px 기준 토스 실측값이지만, 폰트가 달라 nav 텍스트 총 폭이 달라지며
              우측 정렬 기준 시작 위치가 어긋나 Pretendard 적용 후 재측정한 값(294px)으로 보정 */}
