...
**완료 기준**
- [ ] 로컬 `supabase start` → `supabase db push` 마이그레이션 4개 적용 완료
- [ ] `npm run dev` 정상 기동
- [ ] Vercel Preview 배포 성공
...
- **P1-1.** 소셜 OAuth 인증 플로우 `[FE]`
  - `(auth)` 라우트 — 로그인 화면 (카카오·네이버·구글), 본인인증 화면
  - `middleware.ts` — 미인증 시 `/login` 리다이렉트
  - 검증
    - OAuth 로그인 후 `/world` 리다이렉트 성공
