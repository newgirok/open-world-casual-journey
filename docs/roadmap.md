...
- **P2-3.** WASD 이동 + 오솔길 스냅 `[FE]`
  - 키보드 WASD 이동 구현
  - 오솔길 스냅(Snap-to-Trail) 기본 구현 (15m 이내 오솔길 중심선 자석 정렬)
  - 검증
    - 캐릭터 이동 및 오솔길 스냅 동작 확인
...
> 이동 반경을 숲 전체로 확장하고 공간 데이터를 DB에 영구 저장.

- **P3-1.** PostGIS 공간 쿼리 `[DB]`
  - `ST_DWithin`을 이용한 반경 내 스폰서 랜드마크 감지 쿼리
  - GiST 인덱스 기반 성능 튜닝 ([ADR 005](./adr/005-postgis-gist-index.md))
  - 검증
    - 동일 맵 내 10개 스폰서 랜드마크 감지 쿼리가 10ms 이하
...
> 광고주가 셀프 서비스로 숲 속 특정 랜드마크에 브랜드 텍스처를 매핑하고 노출 통계를 확인.

- **P5-1.** 광고주 어드민 포탈 `[FE][BE]`
  - 광고주 어드민 웹 포탈 (숲 지도 위 랜드마크 좌표 선택 + 이미지 업로드 + 원화 결제)
  - 검증
    - 어드민 포탈 정상 접근 및 랜드마크 좌표 등록 성공

- **P5-2.** 브랜드 텍스처 매핑 `[3D]`
  - Three.js TextureLoader로 GLB 랜드마크(그루터기·바위 등) 간판 UV에 브랜드 로고 실시간 매핑
  - 유효 노출 카운팅 (1초 이상 뷰포트 내 완전 진입 시만 DB 기록)
  - 무료 유저 시야 경계(35m)에 스폰서 랜드마크 실루엣 + Glow 효과
  - 검증
