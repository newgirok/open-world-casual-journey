...
위치 기반 근접 탐지 및 스폰서 건물 감지에 **PostGIS + GiST(Generalized Search Tree) 공간 인덱스**를 사용한다. 별도의 Elasticsearch 지오쿼리나 Redis Geospatial을 도입하지 않는다.

## 배경

캐릭터가 이동할 때마다 "내 현재 좌표 반경 R미터 이내에 있는 스폰서 건물·다른 유저를 실시간으로 찾아라"는 공간 연산이 발생한다. 이 연산의 특성은 다음과 같다.
...
| 쿼리 복잡도 | 단순 반경만 가능 | 폴리곤 교차, 도로 스냅 등 복합 가능 |
...
| 연산 | 함수 | 용도 |
|---|---|---|
| 반경 내 스폰서 건물 탐지 | `ST_DWithin` | 광고 노출 감지 |
| 거리 기준 정렬 | `ST_Distance` | LiveKit 구독 Top-8 선정 |
| 도로 스냅 | `ST_ClosestPoint` | GPS 오차 캐릭터 정렬 |
| 건물 충돌 판정 | `ST_Within` | 건물 진입 차단 |
