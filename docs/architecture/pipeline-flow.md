...
유저 입력 (WASD / 조이스틱 / GPS)
  ...
  │
  ▼ [도로 스냅 (Snap-to-Road)]
  ├── ST_ClosestPoint: 반경 15m 내 가장 가까운 도보 도로 중심선으로 자석 정렬
  └── 건물 폴리곤 내부면 충돌 차단
  │
  ▼ [카메라 추적]
...
유저 캐릭터 이동 (50m마다 트리거 또는 실시간)
  ...
  ├── ST_DWithin: 반경 내 활성 스폰서 건물 조회
  └── 결과: {building_id, texture_url, dist_m}[]
  │
  ▼ [클라이언트 렌더링]
  ├── 전방 350~400m 지점 빌딩 에셋 비동기 프리로드
  ├── 가시거리 경계선 도달 전 GLB + 텍스처 메모리 적재 완료
  └── Three.js TextureLoader → 건물 GLB UV에 브랜드 로고 1:1 매핑
  │
  ▼ [유효 노출 판정]
