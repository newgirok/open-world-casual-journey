...
| **로우폴리 (Low-Poly)** | 각지고 간결한 폴리곤으로 구성된 3D 스타일. 캐릭터·건물 에셋 전체에 적용. |
| **가시거리 안개 (Fog of War)** | 캐릭터 중심에서 일정 반경 바깥을 덮는 반투명 안개. 탐험 긴장감과 BM을 연동하는 핵심 메카닉. |
...
| **도로 스냅 (Snap-to-Road)** | GPS 오차 또는 이동 경로 이탈 시 가장 가까운 도보 가능 도로 중심선으로 캐릭터를 자동 정렬하는 보정. |
...
| **CPT (Cost Per Time)** | 기간 고정제 광고 과금 방식. 클릭·행동 기반(CPC/CPA) 대신 "특정 건물 1개월 독점"처럼 기간 단위 정가 판매. |
